...
for x in tr cv tt; do
  ...
  cat $srcdir/wsj0-2mix_${x}.flist | sed 's/\.wav//' | \
    awk -v dir=$wavdir/$x '{printf("%s %s/mix/%s.wav\n", $1, dir, $1)}' | sort > $data/$x/wav.list

  for i in $channels; do
    tgtdir=$data/$x/ch$i
    mkdir -p $tgtdir
    awk -v ch=$i -v tgtwavdir=$wavdir_isolated/$x/mix '{printf("%s_CH%s %s/%s.CH%s.wav\n", $1, ch, tgtwavdir, $1, ch)}' $data/$x/wav.list | \
    awk '{split($1, lst, "_"); spk=substr(lst[1],1,3)"_"substr(lst[3],1,3); print(spk"_"$0)}' | sort > $tgtdir/wav.scp
