...
def get_power_spectral_density_matrix(observation, mask=None, normalize=True, eps=1e-5):
    """
...
def get_gev_vector(target_psd_matrix, noise_psd_matrix):
    ...
    for f in range(bins):
        try:
            ...
            print('NAN error for frequency {}'.format(f))
            beamforming_vector[f, :] = (
                np.ones((sensors,)) / sensors
            )
...
def gev_wrapper_on_masks(mix, noise_mask=None, target_mask=None,
    ...
    if noise_mask is not None:
        noise_mask = np.transpose(noise_mask, (0, 2, 1))
    if target_mask is not None:
        target_mask = np.transpose(target_mask, (0, 2, 1))
